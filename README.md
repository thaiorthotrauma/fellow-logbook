...
- LINE login (via LIFF) gated behind a fellow whitelist, with email + one-time-code verification on first use — see [SETUP.md](./SETUP.md) for how the login flow works and how to configure it.
...
- Cases are stored in Supabase (Postgres), scoped per fellow via Row-Level Security.
